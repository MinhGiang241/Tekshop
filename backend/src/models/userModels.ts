import mongoose from "mongoose";
import bcrypt from "bcryptjs";

const userSchema = new mongoose.Schema(
  {
    name: {
      type: String,
      require: true,
    },
    email: {
      type: String,
      require: true,
      unique: true,
    },
    password: {
      type: String,
      require: true,
    },
    isAdmin: {
      type: Boolean,
      require: true,
      default: false,
    },
    avatar: {
      type: Buffer,
      default:
        "data:image/png;base64,",
    },
    cart: {
      items: [
        {
          productId: {
            type: mongoose.Schema.Types.ObjectId,
            ref: "Product",
            required: true,
          },
          quantity: { type: Number, required: true },
        },
      ],
    },
  },
  {
    timestamps: true,
  }
);

userSchema.methods.matchPassword = async function (enteredPassword: string) {
  return await bcrypt.compare(enteredPassword, (this as any).password);
};

userSchema.methods.updateProfile = async function (
  avatar: any,
  name: string,
  email: string,
  password: any
) {
  if (avatar) {
    (this as any).avatar = avatar;
  }
  (this as any).name = name;
  (this as any).email = email;
  if (password !== "*****") {
    (this as any).password = password;
  }
  return this.save();
};

userSchema.methods.getCart = function (productId: string) {
  return (this as any).cart.items;
};

userSchema.methods.addToCart = function (productId: string, qty: number) {
  // check product exist in cart
  const cartItems = (this as any).cart.items;
  const existedProductIndex = cartItems.findIndex(
    (i: any) => i.productId.toString() === productId
  );

  const updatedCartItems = [...cartItems];

  // if exist quantity + qty
  if (existedProductIndex >= 0) {
    const updateQuantity =
      Number(qty) + parseFloat(updatedCartItems[existedProductIndex].quantity);
    updatedCartItems[existedProductIndex].quantity = updateQuantity;
  } else {
    // if not exist push new product in cart
    updatedCartItems.push({
      productId: productId,
      quantity: qty,
    });
  }

  // update cart
  (this as any).cart = { items: updatedCartItems };
  return this.save();
};

userSchema.methods.deleteCartItem = function (productId: string) {
  const cartItems = (this as any).cart.items;
  const updatedCartItems = [...cartItems];
  // check product exist in cart
  const existedProductIndex = cartItems.findIndex(
    (i: any) => i.productId.toString() === productId
  );

  // if not exist return
  if (existedProductIndex < 0) {
    (this as any).cart = { items: updatedCartItems };
    return this.save();
  } else {
    // if exist remove item
    const newUpdatedCartItems = updatedCartItems.filter(
      (i: any) => i.productId.toString() !== productId
    );

    // update cart
    (this as any).cart = { items: newUpdatedCartItems };
    return this.save();
  }
};

userSchema.methods.clearCart = function () {
  (this as any).cart = { items: [] };
  return this.save();
};

userSchema.pre("save", async function (next) {
  const user = this as any;

  if (user.isModified("password")) {
    user.password = await bcrypt.hash(user.password, 12);
  }

  next();
});

const User = mongoose.model("User", userSchema);

export default User;
